...
**Contributor quick path (prebuilts):** clone → unpack release assets (or run `make prebuilt-libs` once) → `make local/test` or `go test -tags googlesql ./...` as documented in [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md). CI cache layout for Bazel: [`docs/ci-bazel-cache.md`](docs/ci-bazel-cache.md).
