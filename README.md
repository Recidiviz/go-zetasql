...
**Contributor quick path (prebuilts):** clone → unpack release assets (or run `make prebuilt-libs` and `make prebuilt-libs-googlesql-unified` as in [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md)) → **`make local/test`** (same as **`local/test-root-unified`**: `-tags googlesql,googlesql_unified_prebuilt` + prebuilt verifies). CI cache layout for Bazel: [`docs/ci-bazel-cache.md`](docs/ci-bazel-cache.md).
...
**Mold (Linux):** The **`go-googlesql:dev`** image installs **`mold`** and sets **`CGO_LDFLAGS=-fuse-ld=mold`**. On Linux hosts, if **`mold`** is on **`PATH`**, **`make local/build`** / **`make local/test`** pass the same flag for faster linking.
...
**Default Bazel protobuf archive (Linux/macOS):** **`make extract-protobuf-lib`** (alias **`make prebuilt-libs`**) runs [`internal/ccall/go-protobuf/protobuf/extract_protobuf_cgo_lib.sh`](internal/ccall/go-protobuf/protobuf/extract_protobuf_cgo_lib.sh) and produces **`libprotobuf_cgo.a`**.

**Default build/test path:** After **`make prebuilt-libs`** and **`make prebuilt-libs-googlesql-unified`**, **`make local/build`** / **`make local/test`** use **`-tags googlesql,googlesql_unified_prebuilt`** and require **`libprotobuf_cgo.a`** and **`libgooglesql.a`** (see [`docs/link-only-cgo-migration.md`](docs/link-only-cgo-migration.md)). **`make verify-prebuilt-protobuf`** and **`make verify-prebuilt-googlesql-unified`** fail fast if archives are missing. **`make verify-protobuf-tier-b-alignment`** warns when vendored protobuf is still below the Bazel 29.x line (use **`make sync-protobuf-vendor-from-bazel`** + **`go run ./internal/cmd/vendorpatch`** + **`make regenerate-googlesql-cpp-protos`** to align). Full details: [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md), [`docs/native-build-pipeline.md`](docs/native-build-pipeline.md), [`docs/link-only-cgo-migration.md`](docs/link-only-cgo-migration.md). Install-prefix / **pkg-config** template: [`contrib/googlesql.pc.example`](contrib/googlesql.pc.example). Set **`GOOGLESQL_PREBUILT_PREFIX`** when using a consolidated prefix (documented in the `.pc` example); protobuf prebuilts currently use fixed paths under `go-protobuf/protobuf/lib/`.
