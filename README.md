...
**Downstream / CI shells (no direnv):** [`scripts/go-googlesql-stack-bootstrap.sh`](scripts/go-googlesql-stack-bootstrap.sh) exports the same **`CGO_LDFLAGS_ALLOW`**, **`CGO_LDFLAGS`**, **`CGO_CXXFLAGS`**, caches, and **`GOOGLESQL_BUILD_TAGS`** as [`Taskfile.yml`](Taskfile.yml). Use it from sibling repos when running **`go test`** with **`replace`** to this checkout—see [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md#downstream-bootstrap-no-task--no-direnv).
