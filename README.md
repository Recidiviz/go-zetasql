...
**Prebuilt CGO (Tier B, experimental):** After **`make prebuilt-libs`**, use **`make local/test-prebuilt`** or **`make local/build-prebuilt`** to build with **`-tags googlesql,googlesql_tier_b`** (requires **`libprotobuf_cgo.a`** under [`internal/ccall/go-protobuf/protobuf/lib/`](internal/ccall/go-protobuf/protobuf/)). **`make verify-prebuilt-protobuf`** fails fast if the archive is missing. **`make verify-protobuf-tier-b-alignment`** warns when vendored protobuf is still below the Bazel 29.x line (use **`make sync-protobuf-vendor-from-bazel`** + **`go run ./internal/cmd/vendorpatch`** + **`make regenerate-googlesql-cpp-protos`** to align). Full details: [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md), [`docs/native-build-pipeline.md`](docs/native-build-pipeline.md), [`docs/link-only-cgo-migration.md`](docs/link-only-cgo-migration.md). Install-prefix / **pkg-config** template: [`contrib/googlesql.pc.example`](contrib/googlesql.pc.example). Set **`GOOGLESQL_PREBUILT_PREFIX`** when using a consolidated prefix (documented in the `.pc` example); Tier B protobuf currently uses fixed paths under `go-protobuf/protobuf/lib/`.

**Prebuilt Abseil (experimental):** **`make prebuilt-libs-absl`** builds **`libabsl_cgo.a`** under [`internal/ccall/go-absl/lib/`](internal/ccall/go-absl/). **`make local/test-prebuilt-absl`** runs **`go test`** on the migrated packages (default: **`meta/type_traits`**, **`base/config`**, **`utility/utility`**) with **`-tags googlesql,googlesql_tier_b_absl`**. Read [`docs/prebuilt-absl-overlap.md`](docs/prebuilt-absl-overlap.md) before combining with **`-tags googlesql,googlesql_tier_b`** (protobuf Tier B). **Tag matrix / mutual exclusion:** do not pass **`googlesql_tier_b`** and **`googlesql_tier_b_absl`** together in one build — see the canonical table in **`docs/prebuilt-absl-overlap.md`**. **`make verify-tier-b-cgo-policy`** prints the same policy for CI and local preflight.

**Downstream (`go-googlesqlite`, `bigquery-emulator`):** Keep **`replace`** pointed at the same checkout where you ran **`make prebuilt-libs`** / **`make prebuilt-libs-absl`**, and pass the **same** **`-tags`** (plus matching **`CGO_*`** / **`Makefile`-style** `CGO_LDFLAGS_ALLOW`) when enabling Tier B; see [`docs/prebuilt-cgo.md`](docs/prebuilt-cgo.md#downstream-repositories).
