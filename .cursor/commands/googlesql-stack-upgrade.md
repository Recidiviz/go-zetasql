**Cursor:** Use the slash command **`/googlesql-stack-upgrade`** to insert this prompt in chat or Agent.
...
- **Skill `googlesql-stack-debug`** — [`.cursor/skills/googlesql-stack-debug/SKILL.md`](../skills/googlesql-stack-debug/SKILL.md) — Reusable *debugging and testing* rules: classify failures (sync vs link vs codegen vs semantics), **primary `make local/test-root-unified`** (unified prebuilt + link-only CGO) vs legacy `make local/test`, misleading `go test ./...` on `internal/ccall`, CGO cache, memory (`-p 1`, `scripts/cgo-go.sh`), symptom→cause triage. **Apply this skill whenever builds fail during an upgrade** or when triaging CGO/stack issues; it complements this command rather than replacing it.
...
3. **Canonical green definition** — **go-googlesql:** Prefer **`make local/test-root-unified`** (and related prebuilt targets in the README / [`docs/link-only-cgo-migration.md`](../../docs/link-only-cgo-migration.md)) as the **intended** green bar: **`googlesql` + `googlesql_unified_prebuilt`**, prebuilt archives, link-only CGO. **`make local/test`** without unified tags is **legacy / deprecated** for long-term policy but may still match transitional CI (`TESTPKG` defaults to `./`). Do not treat failures from `go test ./...` across every `internal/ccall/...` shard as blocking unless that scope is explicitly in scope (see skill).
...
End-to-end workflow for bumping **google/googlesql** (submodule in go-googlesql) and keeping **go-googlesqlite** and **bigquery-emulator** aligned.
...
- **Phrases:** `googlesql-upgrade to <tag>`, `upgrade googlesql to <tag>`, `bump googlesql to <tag>`.
...
| `GOOGLESQL_ROOT` | Sibling clone of **google/googlesql** or **google/googlesql** for `git log` / diff between release tags |
...
1. `git status`. If dirty: `git stash push -m "wip: pre googlesql upgrade to <tag>"` unless the user forbids stashing.
...
## Phase 1 — Upstream delta (googlesql / googlesql)
...
- Optionally: `git -C "$GOOGLESQL_ROOT" diff --stat <from>..<to>` or path-limited diffs for `googlesql/public`, `googlesql/resolved_ast`, protos, builtins.
...
1. **Submodule:** In `GO_GOOGLESQL_ROOT`, update [internal/cmd/updater/googlesql](../../internal/cmd/updater/googlesql) to tag `<to>` (`git fetch --tags` and `git checkout <to>` inside submodule—**tag tip only**; see [`docs/googlesql-submodule-policy.md`](../../docs/googlesql-submodule-policy.md)), then commit the submodule pointer in the parent when ready. Any CGO-specific GoogleSQL edits go in **`internal/ccall`** / updater overlays / `vendorpatch`, not as extra submodule commits.
2. **Regeneration / vendoring:**
   ...
   - **Flex:** Conflicting `yyFlexLexer` stubs vs `%option yyclass` in generated flex output may require **post-copy fixes** (e.g. [`internal/cmd/updater/main.go`](../../internal/cmd/updater/main.go) `applyPostCopyOverlays`) and generator config (e.g. `GOOGLESQL_PARSER_FLEX_TOKENIZER_SUPPRESS_FLEXLEXER_STUBS` in [`internal/cmd/generator/config.yaml`](../../internal/cmd/generator/config.yaml)) so regenerations stay consistent.
...
4. **Tests:** `CGO_ENABLED=1` with `CXX=clang++` (and ccache/mold on Linux per README). Prefer **`make local/test-root-unified`** / **`make local/compile-root-unified-test`** (or `make local/test` / `make local/build` / `make test/linux` only when matching legacy CI or comparing behavior) with `TESTPKG` narrowed when iterating. Prefer the **root** package gate (`TESTPKG` unset or `./`); see **Failure triage** and skill `googlesql-stack-debug` before interpreting `go test ./...` failures under `internal/ccall`. Do **not** run the heaviest suites in parallel with downstream repos. For memory-constrained machines, use `go test -p 1` and optionally [`scripts/cgo-go.sh`](../../scripts/cgo-go.sh).
...
1. With `replace` deps pointing at local googlesql + googlesqlite, add or extend **integration tests** (e.g. `server/server_test.go`) for new builtins or behaviors.
2. Run emulator tests **last**, **sequentially** (not parallel with full googlesql/googlesqlite test runs).
...
Use [`.cursor/skills/googlesql-stack-debug/SKILL.md`](../skills/googlesql-stack-debug/SKILL.md) for expanded triage and test discipline.

| Symptom | Where to look |
|---------|---------------|
| Many failures only under `go test ./...` / isolated `internal/ccall` packages; root **`make local/test-root-unified`** (or legacy `make local/test`) passes | Often **unsupported** standalone shard builds — confirm CI/Makefile default (`TESTPKG=./`) before “fixing” |
...
| New builtins fail in emulator only | googlesqlite registration vs server query path |
...
Use **`GOOGLESQL_ROOT`** for `git log` / `git diff` between release tags (upstream may be **google/googlesql** or **google/googlesql**; tags like `2023.09.1` should match the submodule release you target).
...
upgrade_repo() {
  ...
  if ! git -C "$root" diff --quiet || ! git -C "$root" diff --cached --quiet; then
    git -C "$root" stash push -m "wip: pre googlesql upgrade to ${tag}"
  fi
...
# git add internal/cmd/updater/googlesql && git commit -m "chore(deps): bump googlesql submodule to ${TO_TAG}"
```
...
```bash
cd "$GO_GOOGLESQL_ROOT"
make local/test-root-unified
# Legacy / transitional CI-style gate (deprecated path): make local/test
# or: make test/linux
# narrow: TESTPKG=./internal/... make local/test-root-unified
...
- Submodule policy (read-only upstream): [docs/googlesql-submodule-policy.md](../../docs/googlesql-submodule-policy.md)
