...
if [[ ! -f "$LIB" ]]; then
	...
	echo "Build it with: task prebuilt:protobuf  (requires bazelisk/bazel and a populated submodule)" >&2
	exit 1
fi
if [[ "$(go env GOOS)" == "linux" ]]; then
	...
	if [[ ! -f "$CXXTIER" ]]; then
		...
		echo "Re-run: task prebuilt:protobuf  (extract script copies Bazel llvm_toolchain libc++)" >&2
		exit 1
	fi
fi
echo "ok: $LIB"
