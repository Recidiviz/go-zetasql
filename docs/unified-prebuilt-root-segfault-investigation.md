...
**Update (2026-04, default `googlesql` amalgamation):** When several CGO packages include the same `export.inc` graph (root `bind.cc`, `public/analyzer`, `parser/parser`, `bison_parser_generated_lib`, `common/errors`), the same `.cc` can be compiled in multiple TUs and Abseil reports **duplicate `ABSL_FLAG` registration** at startup. Mitigations in-tree: pick a single owner per translation unit (`GOOGLESQL_OMIT_*` around `#include "*.cc"` in `export.inc`, or omit the bison shard’s copy of sources already owned by `parser` / `public/analyzer`); for `googlesql_minimized_error_message_tag`, `ABSL_DECLARE_FLAG` in `error_helpers.h` plus `GOOGLESQL_OMIT_PUBLIC_ERROR_HELPERS_ABSL_FLAG_DEF` in secondary TUs keeps a single `ABSL_FLAG` definition while the rest of `error_helpers.cc` still compiles where needed.
