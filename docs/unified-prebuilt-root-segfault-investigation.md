...
**Update (2026-04, legacy non-unified `googlesql` / fat amalgamation):** In the **deprecated** build without `googlesql_unified_prebuilt`, several CGO packages can include the same `export.inc` graph (root `bind.cc`, `public/analyzer`, `parser/parser`, `bison_parser_generated_lib`, `common/errors`), so the same `.cc` is compiled in multiple TUs and Abseil reports **duplicate `ABSL_FLAG` registration** at startup. That mode is not the long-term direction (see [link-only-cgo-migration.md](link-only-cgo-migration.md)). Mitigations in-tree: pick a single owner per translation unit (`GOOGLESQL_OMIT_*` around `#include "*.cc"` in `export.inc`, or omit the bison shard’s copy of sources already owned by `parser` / `public/analyzer`); for `googlesql_minimized_error_message_tag`, `ABSL_DECLARE_FLAG` in `error_helpers.h` plus `GOOGLESQL_OMIT_PUBLIC_ERROR_HELPERS_ABSL_FLAG_DEF` in secondary TUs keeps a single `ABSL_FLAG` definition while the rest of `error_helpers.cc` still compiles where needed.
