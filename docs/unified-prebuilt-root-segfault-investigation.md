...
**Historical note (pre–link-only default):** When multiple CGO packages still pulled overlapping `export.inc` / `.cc` graphs into separate TUs, Abseil could report **duplicate `ABSL_FLAG` registration** at startup. In-tree mitigations (still relevant when touching `export.inc` or shard ownership): pick a single owner per translation unit (`GOOGLESQL_OMIT_*` around `#include "*.cc"` in `export.inc`, or omit the bison shard’s copy of sources already owned by `parser` / `public/analyzer`); for `googlesql_minimized_error_message_tag`, `ABSL_DECLARE_FLAG` in `error_helpers.h` plus `GOOGLESQL_OMIT_PUBLIC_ERROR_HELPERS_ABSL_FLAG_DEF` in secondary TUs keeps a single `ABSL_FLAG` definition while the rest of `error_helpers.cc` still compiles where needed. See [link-only-cgo-migration.md](link-only-cgo-migration.md).
