...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source. **Snapshot (2026-04):** **98** total — **`go-absl`** (94), **`go-algorithms`** (3), **`go-base`** (1). **`./scripts/cgo-shard-inventory.sh --list`** has **no** entries under **`go-googlesql/`** (generated `googlesql/*` shards are link-only; **`go-proto`** differential-privacy protos below are link-only).
- Many **link-only** binds (header comment: `Link-only bind.cc`; snapshot **416** files) — implementations in **`libgooglesql.a`**; CI **`--check`** ensures these never `#include` amalgamated `.cc` bodies.
- **`go-absl/**`** still `#include` `.cc` sources where a separate TU or single-owner rule requires it. **`go-algorithms`** still has **three** shards that compile a `.cc` in the bind TU (`bounded-mean-ci`, `count-tree`, `gaussian-dp-calculator` under `go_internal` / `go-algorithms`); treat as **class A** until `nm` / Bazel closure proves those bodies live in **`libgooglesql.a`** (or extend the unified extract). A **spot-check** of `ar t libgooglesql.a` / `llvm-nm` on a typical prebuilt did **not** show obvious `bounded-mean-ci` / `gaussian-dp-calculator` / `count-tree` object names, so flipping those binds to link-only still needs an **extract / target-list** expansion first—not generator-only edits. **`go-base/logging`** still compiles `base/logging.cc` in the bind TU.
...
**Example B candidates** (non-exhaustive): legacy amalgamation under `go-googlesql` **when** `--list` shows such paths — use Phase 4 before changing `bind_unified_prebuilt_*` imports. **As of the 2026-04 inventory snapshot, `--list` contains no `go-googlesql/` paths**; the next class-B pulls will likely be **go-absl**-scoped audits (respecting time/cctz policy) or **`go-algorithms`** after unified-archive closure for the three remaining `.cc` shards.
