...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source (rerun for current counts; recent snapshot: **~101** total — mostly **`go-absl`**, plus **`go-algorithms`** (6) and **`go-base`** (1); **`go-googlesql`** is link-only for generated shards; **`go-proto`** differential-privacy protos below are link-only).
- Many **link-only** binds (header comment: `Link-only bind.cc`; on the order of **~413** files) — implementations in **`libgooglesql.a`**; CI **`--check`** ensures these never `#include` amalgamated `.cc` bodies.
