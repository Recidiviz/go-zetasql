...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source. **Snapshot (2026-04):** **10** total — **`go-absl`** (10) only; **`go-base`** has **no** such shards after **`base/logging`** went link-only. **`./scripts/cgo-shard-inventory.sh --list`** has **no** entries under **`go-googlesql/`**, **`go-algorithms/`**, or **`go-base/`** (generated `googlesql/*` shards are link-only; **`go-proto`** differential-privacy protos below are link-only). The **10** remaining amalgamation binds are: **`debugging/{failure_signal_handler,leak_check_disable}`**, **`log/{flags,scoped_mock_log}`**, **`status/status_matchers`**, **`strings/cordz_sample_token`**, **`synchronization/kernel_timeout_internal`**, **`time/go_internal/cctz/{civil_time,time_zone}`**, and **`time/time`**.
...
**Likely next steps (priority order; snapshot **10** amalgamation binds under **`go-absl`** — see §Baseline metrics and **Eighth follow-up** above):**

1. **Prebuilt expansion** for shards where **`nm`** / **`ar t`** show **no** archive TU (**`log/flags`**, **`cordz_sample_token`**, **`failure_signal_handler`**, **`leak_check_disable`**): extend Bazel collection in **`extract_protobuf_cgo_lib.sh`**, rebuild **`libprotobuf_cgo.a`**, duplicate-global check, then **`exclude_amalgamation_sources`**.
