...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source. **Snapshot (2026-04):** **4** total — **`go-absl`** (4) only; **`go-base`** has **no** such shards after **`base/logging`** went link-only. **`./scripts/cgo-shard-inventory.sh --list`** has **no** entries under **`go-googlesql/`**, **`go-algorithms/`**, or **`go-base/`** (generated `googlesql/*` shards are link-only; **`go-proto`** differential-privacy protos below are link-only). The **4** remaining amalgamation binds are: **`debugging/leak_check_disable`** (vendored TU), **`log/scoped_mock_log`**, **`status/status_matchers`**, and **`time/go_internal/cctz/time_zone`** (IANA zone bodies; **Tenth follow-up** retired **`civil_time`**, **`absl/time/time`**, **`kernel_timeout_internal`** via **`libprotobuf_cgo.a`** + **`exclude_amalgamation_sources`**).
...
- **Time / cctz / `kernel_timeout_internal` (extract alignment):** The extract script **drops** cctz **`time_zone_*.pic.o`** / **`zone_info_source.pic.o`** from **`libprotobuf_cgo.a`** so **IANA zone** implementation stays **single-owner** with the **`go-absl/time/.../cctz/time_zone`** amalgamation TU (see script comments and [unified-prebuilt-root-segfault-investigation.md](unified-prebuilt-root-segfault-investigation.md)). **`exclude_amalgamation_sources`** for **`time_zone`** still requires **not** applying YAML-only excludes without an extract / ownership change (**Tenth follow-up** landed the safe subset: **`civil_time`**, **`absl/time/time`**, **`kernel_timeout_internal`** only where **`*.pic.o`** are already merged).
...
- **Tenth follow-up (7 → 4, Option B):** **`cclib.exclude_amalgamation_sources`** for **`absl/time/internal/cctz/civil_time`** (**`civil_time_detail.cc`**), **`absl/time/time`** (**`civil_time.cc`**, **`clock.cc`**, **`duration.cc`**, **`format.cc`**, **`time.cc`**), and **`absl/synchronization/kernel_timeout_internal`** (**`kernel_timeout.cc`**) — implementations come from **`libprotobuf_cgo.a`** (matching **`*.pic.o`** via transitive Abseil protobuf deps; **`task verify:prebuilt-protobuf`**). **`absl/time/internal/cctz/time_zone`** stays amalgamated: [extract_protobuf_cgo_lib.sh](../internal/ccall/go-protobuf/protobuf/extract_protobuf_cgo_lib.sh) still **drops** **`time_zone_*.pic.o`** / **`zone_info_source.pic.o`** so the Go shard remains single-owner for IANA zone code ([unified-prebuilt-root-segfault-investigation.md](unified-prebuilt-root-segfault-investigation.md)). **Option A** (archive-owned IANA, relax **`grep -Ev`**) remains a separate epic if inventory must drop **`time_zone`** too.
...
**After successive consolidation waves through 2026-04:** `./scripts/cgo-shard-inventory.sh --summary` reports **4** `bind.cc` files that still `#include` a `.cc` — all under **`go-absl`** (see §Baseline metrics). **`go-googlesql/`**, **`go-base/`**, and **`go-algorithms/`** remain clear on `--list`.

**Likely next steps (priority order; snapshot **4** amalgamation binds under **`go-absl`** — see §Baseline metrics):**

1. **Prebuilt expansion (partial):** **`extract_protobuf_cgo_lib.sh`** builds **`@com_google_absl//absl/log:flags`**, **`//absl/strings:cordz_sample_token`**, **`//absl/debugging:failure_signal_handler`** so those **`.pic.o`** merge into **`libprotobuf_cgo.a`**; **`exclude_amalgamation_sources`** applied in [config.yaml](../internal/cmd/generator/config.yaml). **`leak_check_disable`** stays amalgamated (**vendored** [`internal/ccall/absl/debugging/leak_check_disable.cc`](../internal/ccall/absl/debugging/leak_check_disable.cc), not an upstream Abseil shard).
2. **cctz `time_zone` (Option A epic):** follow the **cctz / `kernel_timeout` future PR checklist** — **`Tenth follow-up`** retired **`civil_time`**, **`absl/time/time`**, and **`kernel_timeout_internal`**; **`time_zone`** IANA bodies still amalgamated until extract policy flips (see bullet above).
