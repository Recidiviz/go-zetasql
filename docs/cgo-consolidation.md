...
- **Consolidation iteration (2026-04-13):** Baseline reconfirmed (**2** `#include "*.cc"` binds under **`go-absl`** only; **420** link-only). `./scripts/cgo-shard-inventory.sh --check` green; `task test:compile-root-unified` and `task test:googlesql-unified-root` green; `scripts/verify-prebuilt-protobuf.sh` and `scripts/verify-prebuilt-googlesql-unified.sh` green (including duplicate `(T)` symbol check vs **`libprotobuf_cgo.a`**). No generator or `bind_unified_prebuilt_*` edits; inventory **2→0** remains blocked on GMock policy (§Open product decisions).
...
- **Twelfth follow-up (3 → 2):** Vendored [**`internal/ccall/absl/debugging/leak_check_disable.cc`**](../internal/ccall/absl/debugging/leak_check_disable.cc) — [extract_protobuf_cgo_lib.sh](../internal/ccall/go-protobuf/protobuf/extract_protobuf_cgo_lib.sh) compiles it to **`vendored_leak_check_disable.pic.o`** and merges into **`libprotobuf_cgo.a`**; **`cc_library`** **`leak_check_disable`** in [internal/ccall/absl/debugging/BUILD.bazel](../internal/ccall/absl/debugging/BUILD.bazel) registers the package for the generator; **`cclib.exclude_amalgamation_sources`** for **`absl/debugging/leak_check_disable`** + **`go run ./internal/cmd/generator`**.
...
- **Optional — `leak_check_disable` .pic.o merge:** **Done (Twelfth follow-up)** — see bullet above.
...
| After **`extract_protobuf_cgo_lib.sh`** changes | **`task prebuilt:protobuf`**, **`task verify:prebuilt-protobuf`**, **`task test:local TESTPKG=./`** — **`libprotobuf_cgo.a`** is gitignored ([prebuilt-cgo.md](prebuilt-cgo.md)); script also compiles vendored **`leak_check_disable.cc`** into the archive (Twelfth follow-up) |
