...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source. **Snapshot (2026-04):** **97** total — **`go-absl`** (94), **`go-algorithms`** (2), **`go-base`** (1). **`./scripts/cgo-shard-inventory.sh --list`** has **no** entries under **`go-googlesql/`** (generated `googlesql/*` shards are link-only; **`go-proto`** differential-privacy protos below are link-only).
- Many **link-only** binds (header comment: `Link-only bind.cc`; snapshot **417** files) — implementations in **`libgooglesql.a`**; CI **`--check`** ensures these never `#include` amalgamated `.cc` bodies.
- **`go-absl/**`** still `#include` `.cc` sources where a separate TU or single-owner rule requires it. **`go-algorithms`** still has **two** shards that compile a `.cc` in the bind TU (**`bounded-mean-ci`**, **`count-tree`** under `go_internal`); **`gaussian-dp-calculator`** is **link-only** after adding **`@com_google_cc_differential_privacy//algorithms:gaussian-dp-calculator`** to [default_bazel_targets.txt](../internal/ccall/go-googlesql-unified/default_bazel_targets.txt) and listing **`algorithms/gaussian-dp-calculator`** under `cclib.link_only_bind_packages`. Rebuild **`libgooglesql.a`** (`task prebuilt:googlesql-unified`) so `bounded-mean-ci.pic.o` / `count-tree.pic.o` / `gaussian-dp-calculator.pic.o` are collected; then apply the same link-only generator pattern for the remaining two shards when ready. **`go-base/logging`** still compiles `base/logging.cc` in the bind TU.
...
**Example B candidates** (non-exhaustive): legacy amalgamation under `go-googlesql` **when** `--list` shows such paths — use Phase 4 before changing `bind_unified_prebuilt_*` imports. **As of the 2026-04 inventory snapshot, `--list` contains no `go-googlesql/` paths**; remaining **`.cc` includes** under **`go-algorithms`** are **`bounded-mean-ci`** and **`count-tree`** (after **`gaussian-dp-calculator`** link-only). Broader pulls remain **go-absl**-scoped audits (respecting time/cctz policy).
...
- Remaining **`go-algorithms`** shards that still `#include` a `.cc` in `bind.cc` are **`bounded-mean-ci`** and **`count-tree`** — same external-repo targets are listed in **`default_bazel_targets.txt`** for archive closure; flip to link-only with **`algorithms/internal/bounded-mean-ci`** / **`algorithms/internal/count-tree`** in `cclib.link_only_bind_packages` + exportinc filter when ready.
