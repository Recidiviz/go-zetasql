...
- **`./scripts/cgo-shard-inventory.sh --summary`** reports how many `bind.cc` files anywhere under `internal/ccall/` still `#include` a `.cc` source. **Snapshot (2026-04):** **7** total — **`go-absl`** (7) only; **`go-base`** has **no** such shards after **`base/logging`** went link-only. **`./scripts/cgo-shard-inventory.sh --list`** has **no** entries under **`go-googlesql/`**, **`go-algorithms/`**, or **`go-base/`** (generated `googlesql/*` shards are link-only; **`go-proto`** differential-privacy protos below are link-only). The **7** remaining amalgamation binds are: **`debugging/leak_check_disable`** (vendored TU), **`log/scoped_mock_log`**, **`status/status_matchers`**, **`synchronization/kernel_timeout_internal`**, **`time/go_internal/cctz/{civil_time,time_zone}`**, and **`time/time`**.
...
- **Seventh follow-up (11 → 10):** **`absl/debugging/examine_stack`** — `ar t libprotobuf_cgo.a` includes **`examine_stack*.pic.o`**; **`cclib.exclude_amalgamation_sources`** for **`absl/debugging/internal/examine_stack.cc`** so the CGO TU no longer compiles that body (implementation from the archive). **`absl/log/flags`** (`absl/log/flags.cc`), **`absl/strings/internal/cordz_sample_token.cc`**, **`absl/debugging/failure_signal_handler.cc`**, and **`absl/debugging/leak_check_disable.cc`** — **archive proof failed**: no distinct matching **`.pic.o`** names in **`libprotobuf_cgo.a`** / **`libgooglesql.a`** under `ar t` / substring checks comparable to other Abseil objects, so **`exclude_amalgamation_sources`** would risk **undefined symbols**; leave amalgamated until prebuilts include those TUs or ownership changes.
...
- **Time / cctz / `kernel_timeout_internal` (extract alignment):** The same extract script **drops** cctz **`time_zone_*.pic.o`** / **`zone_info_source.pic.o`** from **`libprotobuf_cgo.a`** so **IANA zone** implementation stays **single-owner** with the **`go-absl/time/.../cctz/time_zone`** amalgamation TU (see script comments and [unified-prebuilt-root-segfault-investigation.md](unified-prebuilt-root-segfault-investigation.md)). **`exclude_amalgamation_sources`** for **`time/time`**, **`civil_time`**, **`time_zone`**, or **`kernel_timeout_internal`** is **not** a standalone YAML change: it requires a coordinated update to **which objects are merged into `libprotobuf_cgo.a`**, regeneration of prebuilts, **`nm`** duplicate-symbol review, and **`task test:local`**. Until then, keep the deferral comment in [config.yaml](../internal/cmd/generator/config.yaml).

- **Eighth follow-up (`nm` verification, 2026-04):** On checked-in **`libprotobuf_cgo.a`** (`linux_amd64`), **`nm -C`** plus substring search found **no** symbols suggestive of **`failure_signal_handler`** or **`leak_check_disable`** TUs (e.g. no **`FailureSignal`**, **`InstallFailureSignal`**, **`LeakCheckDisabl`** matches). **`SampleToken`** / **`sample_token`** symbol names did not appear; **`cordz_next_sample`** / related symbols come from other cordz objects already in the archive. **`absl/log/flags.cc`** still has no obvious dedicated TU footprint beyond log helpers merged elsewhere. Conclusion: **`ar t`** naming and **`nm`** both support keeping those four bodies **amalgamated** unless **`extract_protobuf_cgo_lib.sh`** is extended to merge the missing **`.pic.o`** files and prebuilts are rebuilt (**prebuilt expansion** path).
...
- **cctz / `kernel_timeout` future PR checklist:** (1) Read [unified-prebuilt-root-segfault-investigation.md](unified-prebuilt-root-segfault-investigation.md) and the **`grep -Ev 'time_zone_…'`** block in [extract_protobuf_cgo_lib.sh](../internal/ccall/go-protobuf/protobuf/extract_protobuf_cgo_lib.sh). (2) Decide single owner: **archive** vs **go-absl** IANA + civil_time + **`kernel_timeout_internal`**. (3) Adjust extract filters and/or **`go-absl/time`** blank-import order; rebuild **`libprotobuf_cgo.a`**. (4) Run **`scripts/verify-prebuilt-protobuf.sh`**, **`scripts/verify-prebuilt-googlesql-unified.sh`**, **`./scripts/cgo-shard-inventory.sh --check`**, **`task test:local TESTPKG=./`**. (5) Only then add **`exclude_amalgamation_sources`** for the time-related **`pkg:`** keys. **Status:** not executed in the default tree yet; YAML-only excludes previously caused **undefined cctz** symbols — requires the coordinated extract/prebuilt change above.
...
**After successive consolidation waves through 2026-04:** `./scripts/cgo-shard-inventory.sh --summary` reports **7** `bind.cc` files that still `#include` a `.cc` — all under **`go-absl`** (see §Baseline metrics). **`go-googlesql/`**, **`go-base/`**, and **`go-algorithms/`** remain clear on `--list`.

**Likely next steps (priority order; snapshot **7** amalgamation binds under **`go-absl`** — see §Baseline metrics):**

1. **Prebuilt expansion (partial):** **`extract_protobuf_cgo_lib.sh`** now builds **`@com_google_absl//absl/log:flags`**, **`//absl/strings:cordz_sample_token`**, **`//absl/debugging:failure_signal_handler`** so those **`.pic.o`** merge into **`libprotobuf_cgo.a`**; **`exclude_amalgamation_sources`** applied in [config.yaml](../internal/cmd/generator/config.yaml). **`leak_check_disable`** stays amalgamated (**vendored** [`internal/ccall/absl/debugging/leak_check_disable.cc`](../internal/ccall/absl/debugging/leak_check_disable.cc), not an upstream Abseil shard).
2. **cctz / time / `kernel_timeout_internal`:** follow the **cctz future PR checklist** bullet above (extract + prebuilts + tests; not YAML-only).
3. **GMock shards:** policy is **keep generated, optional consumers** — do not expect inventory zero without removing test-oriented packages.
4. **`googlesql_tier_b_absl`** + **`libabsl_cgo.a`** stays a **separate Tier B pilot** — not a substitute for shrinking the default inventory ([tier-b-absl-protobuf.md](tier-b-absl-protobuf.md)).
