...
This note tracks **upstream** changes between tags `2023.09.1` and `2023.10.1` in [ZetaSQL](https://github.com/google/zetasql) (`zetasql/` at those tags) and how they relate to **go-zetasql**, **go-zetasqlite**, and **bigquery-emulator**.
...
Refresh `internal/ccall/zetasql` with [`internal/cmd/updater`](../internal/cmd/updater) after bumping the submodule to `2023.10.1`. Prefer `GO_ZETASQL_SKIP_PROTOBUF_COPY=1` when protobuf vendoring should stay on the existing pin ([`docs/protobuf-vendoring.md`](protobuf-vendoring.md)). Then run `go run .` from [`internal/cmd/generator`](../internal/cmd/generator) so CGO amalgamation and generated Go stay aligned.
...
No new **`builtin_function.proto`** enum values in this tag range (per upstream diff). **go-zetasqlite** may only need **LanguageFeature** or analyzer tweaks if tests surface new resolved nodes or option syntax; otherwise verify with existing tests. **bigquery-emulator** inherits behavior through the driver; add HTTP/query tests if user-visible SQL surface expands.
