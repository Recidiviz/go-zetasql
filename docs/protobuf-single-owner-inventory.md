...
- **~150+** `export.inc` files under `internal/ccall/go-zetasql/` reference
  `#include "go-protobuf/protobuf/export.inc"` (plus parallel paths under
  `legacy_zetasql/`).
- **780+** `bind.cc` files under `internal/ccall/go-zetasql/`.
...
- `make local/test` (root package), then go-zetasqlite / bigquery-emulator with
