...
## Upstream themes → go-zetasql / go-zetasqlite / emulator

### JSON (`zetasql/public/functions/json.cc`, `json_internal.h`, `reference_impl/functions/json.cc`)

| Upstream change (2023.03.2→2023.04.1) | go-zetasql | go-zetasqlite | Notes |
|--------------------------------------|------------|---------------|-------|
| **`JsonPathEvaluator`**: escaping callback `(string_view)` → `(string_view, bool is_key)`; legacy one-arg callback retained | Vendored C++ in [`internal/cmd/updater/zetasql/`](../internal/cmd/updater/zetasql/) tracks this tree when updated | [`internal/function_json.go`](../../go-zetasqlite/internal/function_json.go) uses **goccy/go-json** paths, not the C++ JSONPath extractor—**behavioral parity** for “escape needed” callbacks is **not** 1:1 with C++; re-check if you add native JSONPath |
| **`JsonArray` / `JsonObject` helpers** (variadic `Value` → JSON array/object; duplicate keys skip later values) | N/A (runtime in reference impl) | **`JSON_ARRAY`**, **`JSON_OBJECT`** in [`function_json.go`](../../go-zetasqlite/internal/function_json.go); duplicate keys **first wins** (matches upstream) | Covered by [`query_test.go`](../../go-zetasqlite/query_test.go) (`json_object_duplicate_keys_first_wins`, `json_array_*`) |
| **Reference impl**: `JsonExtractFunction` rename from `JsonFunction`; **`ConvertJsonFunction`** (INT64/DOUBLE/BOOL from JSON), **`ConvertJsonLaxFunction`**, **`JsonTypeFunction`**, **`JsonArrayFunction`** (`JSON_ARRAY` builtin) | Analyzer emits `FunctionKind`s; protos in [`builtin_function.proto`](../internal/cmd/updater/zetasql/zetasql/public/builtin_function.proto) | [`function_register.go`](../../go-zetasqlite/internal/function_register.go): `int64`, `json_type`, `lax_*`, `json_array`, etc. | End-to-end JSON cast tests in `query_test.go` (`json_int64`, `json_bool`, `json_float64`, `json_type`, LAX tests) |

### RANGE (`zetasql/public/functions/range.cc` **new**)

| Upstream | go-zetasqlite |
|----------|----------------|
...
| Item | Proto / feature in vendored tree | go-zetasqlite runtime |
|------|-----------------------------------|------------------------|
| **`GROUPING()`** scalar | `FN_GROUPING` in [`builtin_function.proto`](../internal/cmd/updater/zetasql/zetasql/public/builtin_function.proto); `FEATURE_V_1_4_GROUPING_BUILTIN` in [`options.proto`](../internal/cmd/updater/zetasql/zetasql/public/options.proto) | **Not** registered in [`function_register.go`](../../go-zetasqlite/internal/function_register.go). Aggregate path documents `GROUPING SETS` only in [`transformer_scan_aggregate.go`](../../go-zetasqlite/internal/transformer_scan_aggregate.go) |
| **`ARRAY_ZIP_MODE`** (opaque enum) | [`array_zip_mode.proto`](../internal/cmd/updater/zetasql/zetasql/public/functions/array_zip_mode.proto) | **`ARRAY_ZIP` not** implemented; named-argument / `AS` alias form for zip args appears in upstream parser tests |
| **Function call arguments with alias** | Parser: `ASTExpressionWithAlias` in [`zetasql/parser/bison_parser.y`](../internal/cmd/updater/zetasql/zetasql/parser/bison_parser.y), tests `function_call_argument_alias.test` | **Supported** wherever the **linked ZetaSQL/GooglesSQL library** version supports it (go-zetasql uses CGO + vendored parser). Resolver rejects invalid alias use at analysis time |
...
- SQL parity is inherited from **go-zetasqlite**; add **integration** tests in [`server/server_test.go`](../../bigquery-emulator/server/server_test.go) for critical builtins (see `TestSimpleQuery`).
...
Heavy **`go test ./...`** runs can **OOM** (`clang++: signal: killed`) when many CGO packages compile at once. Align with the [build-speed / Makefile](../Makefile) setup in **go-zetasql**:

- Prefer **`make -C ../go-zetasql local/test`** with `TESTPKG` set to a **single package** (default `./`), or **`make test/linux`** inside **`go-zetasql:dev`** so **GOCACHE**, **GOMODCACHE**, and **ccache** stay warm.
- Use **low `-p`** (the Makefile sets **`GO_BUILD_P_MAX`** default **2**; use **`GO_BUILD_P=1`** or **`go test -p 1`** on tight hosts).
- When **`mold`** is on `PATH`, **`make local/build`** / **`local/test`** pass **`CGO_LDFLAGS=-fuse-ld=mold`** for faster linking (Linux).

**go-zetasqlite** / **bigquery-emulator**: run targeted tests on the **root or one package** (e.g. `go test -p 1 -run 'TestQuery/json_float64_wide_number_mode' .` in go-zetasqlite) with the same **`CC="ccache clang"`** / **`GO_CACHE_ROOT`** env as above—not unbounded `./...` without Docker or a large machine.
