...
# Custom {{ define "stackEntry" }} below uses a union; must match non-variant codegen.
variantStackEntry = false
...
/* Requiring a newline at the end of dash_coment and pound_comment does not
   ...
   https://github.com/google/googlesql/blob/master/zetasql/parser/flex_tokenizer.h?l=128).
*/
/* Dash comments using -- */
dash_comment          = /\-\-[^\r\n]*(\r|\n|\r\n)?/
...
pre_statement:
    %empty
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_BEGIN, LB_BEGIN_AT_STATEMENT_START); }
    ;
...
statement_level_hint {node}:
  hint
    {
      TM_OVERRIDE_CURRENT_TOKEN_LOOKBACK(@hint, LB_END_OF_STATEMENT_LEVEL_HINT);
      $$ = $hint;
    }
  ;
...
unterminated_script_statement {node}:
    ...
    | label ":"
      ...
      { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_BEGIN, LB_OPEN_STATEMENT_BLOCK); }
...
aux_load_data_statement {node}:
    ...
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(LPAREN, LB_LPAREN_NOT_OPEN_SUBQUERY); }
...
as_before_query {node}:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_AS, LB_AS_BEFORE_QUERY); } "AS"
    ;
...
column_list {node}:
    column_list_prefix
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(RPAREN, LB_CLOSE_COLUMN_LIST); }
    ")"
      {
        $$ = parser->WithEndLocation($1, @$);
      }
...
set_operation_metadata {node}:
    ...
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_ALL, LB_SET_OP_QUANTIFIER);
      TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_DISTINCT, LB_SET_OP_QUANTIFIER);}
...
pre_select_with:
  %empty
  { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_WITH, LB_WITH_IN_SELECT_WITH_OPTIONS); }
  ;
...
hint {node}:
    KW_OPEN_INTEGER_HINT integer_literal
      {
        ABORT_CHECK(@$, TM_PARSER_LA_IS_EMPTY(),
...
graph_for_operator {node}:
  "FOR" identifier { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_IN, LB_GRAPH_FOR_IN); }
  "IN" expression
...
aliased_query {node}:
    identifier "AS" "(" query
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(RPAREN, LB_CLOSE_ALIASED_QUERY); }
...
template_type_open:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(LT, LB_OPEN_TYPE_TEMPLATE); }
    "<"[open]
    { @$ = @open; }
    ;

template_type_close:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(GT, LB_CLOSE_TYPE_TEMPLATE); }
    ">"[close]
    { @$ = @close; }
    ;
    ...
    "INSERT" opt_or_ignore_replace_update[insert_mode] opt_into maybe_dashed_generalized_path_expression[insert_target] opt_hint
      {
        ...
        TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(LPAREN, LB_PAREN_OPENS_QUERY);
...
nested_dml_statement {node}:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(LPAREN, LB_OPEN_NESTED_DML); }
    "("[open] dml_statement ")"
      {
        $$ = $dml_statement;
      }
...
loop_statement {node}:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_LOOP, LB_OPEN_STATEMENT_BLOCK); }
...
while_statement {node}:
    "WHILE" expression { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_DO, LB_OPEN_STATEMENT_BLOCK); } "DO" statement_list "END" "WHILE"
    {
...
repeat_statement {node}:
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_REPEAT, LB_OPEN_STATEMENT_BLOCK); }
...
for_in_statement {node}:
    ...
    { TM_OVERRIDE_NEXT_TOKEN_LOOKBACK(KW_DO, LB_OPEN_STATEMENT_BLOCK); } "DO" statement_list "END" "FOR"
    {
     if (!parser->language_options().LanguageFeatureEnabled(
...
next_statement_kind {ast_node_kind}:
    ...
    | hint { TM_OVERRIDE_CURRENT_TOKEN_LOOKBACK(@hint, LB_END_OF_STATEMENT_LEVEL_HINT); }
      next_statement_kind_without_hint[kind]
      {
...
struct symbol {
  ...
  {{ template "LocationClass" }} location = {{ template "LocationClass" }}();
};
{{ end -}}
