...
#ifndef THIRD_PARTY_ZETASQL_ZETASQL_BASE_BITS_H_
#define THIRD_PARTY_ZETASQL_ZETASQL_BASE_BITS_H_
...
class Bits {
 public:
  ...
  static typename UnsignedType<T>::Type GetBits(const T src,
    ...
    ABSL_DCHECK_GT(sizeof(UnsignedT) * 8, offset);
    ABSL_DCHECK_GE(sizeof(UnsignedT) * 8, offset + nbits);
  ...
  static typename UnsignedType<T>::Type GetLowBits(const T src,
    ...
    typedef typename UnsignedType<T>::Type UnsignedT;
    const UnsignedT unsigned_src = absl::bit_cast<UnsignedT>(src);
    ABSL_DCHECK_GE(sizeof(UnsignedT) * 8, nbits);
...
inline uint32_t Bits::ReverseBits32(uint32_t n) {
  ...
  return zetasql_base::gbswap_32(n);
#endif
}
...
#endif  // THIRD_PARTY_ZETASQL_ZETASQL_BASE_BITS_H_
