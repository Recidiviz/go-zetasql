...
const LibSpec = struct {
    ...
    // Additional sources outside `root` (or excluded by the filters
    // above) that the library still needs.
...
const lib_specs = [_]LibSpec{
    .{
        ...
        .root = ccall ++ "/zetasql",
        .excludes = &.{
            // Test-only and unused subsystems.
            "_test",           "unittest",   "test_util",
            "/testdata/",      "/testing/",  "/compliance/",
            "/local_service/", "/tools/",    "/jdk/",
            "/scripting/",     "/examples/", "/reference_impl/",
            "benchmark",       "fuzz",
            // Requires Google's differential-privacy library, which is
            // not vendored (the cgo build never compiles it either).
            "anonymization_utils",
            // Generated by a newer protoc than the vendored protobuf
            // runtime; nothing outside these files references them.
            "type_modifiers",
        },
        .extras = &.{
            // The resolver instantiates SampleAnnotation and resolves
            // CREATE PROCEDURE bodies via ParsedScript, so these two
